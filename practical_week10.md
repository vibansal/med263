...
What is the most constrained gene in the list (high pLI score) ? Does this gene have a disease association in humans (https://www.omim.org/entry/603732) ?
...
```Shell
grep SLC30A8 GTEx_Analysis_v6p_RNA-seq_RNA-SeQCv1.1.8_gene_median_rpkm.gct
```
...
From the output, we observe that the all the 103 heterozygous variants could be assembled into a single haplotype block. This is due to the long read lengths of the PacBio reads (5-20 kb) compared to Illumina reads (100 bp reads, 300-500 bp fragments). 
